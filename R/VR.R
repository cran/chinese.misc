...
#'
#' @export
VR <- function(tofactor = 0, keepblank = 0, sep = "\t") {
    ...
    if((identical(keepblank, 1) | identical(keepblank, TRUE)) & asfa == FALSE)
        nast <- "NA"
    utils::read.table("clipboard", header = FALSE, row.names = 1, blank.lines.skip = FALSE, sep = sep, na.strings = nast, stringsAsFactors = asfa)
}
